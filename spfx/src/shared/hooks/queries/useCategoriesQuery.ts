...
export function useCategoriesQuery(): {
  ...
  const [state, setState] = React.useState<QueryState<ICategory[]>>(() => {
    const cached = services.cache.get<ICategory[]>(cacheKey);
    if (cached) {
  ...
  const fetch = React.useCallback(async (): Promise<void> => {
    ...
    if (result.success) {
      services.cache.set(cacheKey, result.data, CACHE_TTLS.CATEGORIES);
      // eslint-disable-next-line require-atomic-updates
      hasDataRef.current = true;
